...
const Menu = (props) => {
  ...
  const anecdote = match 
    ? props.anecdotes.find(note => note.id === Number(match.params.id))
    : null
  const padding = {
    paddingRight: 5
  }
  return (
    <div>
      ...
      <Routes>
        <Route path='/new' element={<CreateNew addNew = {props.addNew} /> }></Route>
...
const Anecdote = ({ anecdote }) => {
  ...
  // const anc = anecdotes.find( a => a.id === Number(id))
  return (
  <div>
      <h2>{anecdote.content}</h2>
      <div>has {anecdote.votes} votes</div>
      <div> for more information read <a href = {anecdote.info} >here</a></div>
  </div>)
...
const App = () => {
  ...
  return (
    <div>
      {
        notification ? <p> {notification} </p> : <p></p>
      }
